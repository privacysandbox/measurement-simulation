...
public final class AdServicesStatusUtils {
  ...
  /**
   * The status code has not been set. Keep unset status code the lowest value of the status codes.
   */
   ...
   *
   * <p>This error may be considered similar to {@link TimeoutException}
   */
  public static final int STATUS_TIMEOUT = 13;

  /**
   * The device is not running a version of WebView that supports JSSandbox, required for FLEDGE Ad
   * Selection.
   *
   * <p>This error may be considered similar to {@link IllegalStateException}.
   */
  ...
  /**
   * The error message to be returned along with {@link SecurityException} when call failed because
   * it crosses user boundaries.
   */
  ...
  /**
   * The error message returned when a call to schedule a custom audience update fails because of an
