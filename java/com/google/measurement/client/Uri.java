...
public abstract class Uri implements Comparable<Uri> {
  ...
  */
  ...
  /**
   * Holds a placeholder for strings which haven't been cached. This enables us to cache null. We
   * intentionally create a new String instance so we can compare its identity and there is no
   * chance we will confuse it with user data.
   *
   * <p>NOTE This value is held in its own Holder class is so that referring to {@link
   * NotCachedHolder#NOT_CACHED} does not trigger {@code Uri.<clinit>}. For example, {@code
   * PathPart.<init>} uses {@code NotCachedHolder.NOT_CACHED} but must not trigger {@code
   * Uri.<clinit>}: Otherwise, the initialization of {@code Uri.EMPTY} would see a {@code null}
   * value for {@code PathPart.EMPTY}!
   *
   * @hide
   */
  ...
  /**
   * Returns true if this URI is opaque like "mailto:nobody@google.com". The scheme-specific part of
   * an opaque URI cannot start with a '/'.
   */
  ...
  /**
   * Gets the scheme-specific part of this URI, i.e.&nbsp;everything between the scheme separator
   * ':' and the fragment separator '#'. If this is a relative URI, this method returns the entire
   * URI. Decodes escaped octets.
   *
   ...
   * Gets the scheme-specific part of this URI, i.e.&nbsp;everything between the scheme separator
   * ':' and the fragment separator '#'. If this is a relative URI, this method returns the entire
   * URI. Leaves escaped octets intact.
   *
   * <p>Example: "//www.google.com/search?q=android"
   *
  ...
  /**
   * Gets the decoded authority part of this URI. For server addresses, the authority is structured
   * as follows: {@code [ userinfo '@' ] host [ ':' port ]}
   *
  ...
  /**
   * Gets the encoded authority part of this URI. For server addresses, the authority is structured
   * as follows: {@code [ userinfo '@' ] host [ ':' port ]}
   *
   * <p>Examples: "google.com", "bob@google.com:80"
   *
  ...
  /**
   * Gets the decoded user information from the authority. For example, if the authority is
   * "nobody@google.com", this method will return "nobody".
   *
  ...
  /**
   * Gets the encoded user information from the authority. For example, if the authority is
   * "nobody@google.com", this method will return "nobody".
   *
   * @return the user info for this URI or null if not present
   */
  ...
  /**
   * Gets the encoded host from the authority for this URI. For example, if the authority is
   * "bob@google.com", this method will return "google.com".
   *
  ...
  /**
   * Gets the encoded query component from this URI. The query comes after the query separator ('?')
   * and before the fragment separator ('#'). This method would return "q=android" for
   * "http://www.google.com/search?q=android".
   *
  ...
  /**
   * Compares this Uri to another object for equality. Returns true if the encoded string
   * representations of this Uri and the given Uri are equal. Case counts. Paths are not normalized.
   * If one Uri specifies a default port explicitly and the other leaves it implicit, they will not
   * be considered equal.
   */
  ...
  /**
   * Return a string representation of this URI that has common forms of PII redacted, making it
   * safer to use for logging purposes. For example, {@code tel:800-466-4411} is returned as {@code
   * tel:xxx-xxx-xxxx} and {@code http://example.com/path/to/item/} is returned as {@code
   * http://example.com/...}. For all other uri schemes, only the scheme, host and port are
   * returned.
   *
  ...
  /**
   * Creates a Uri from a file. The URI has the form "file://<absolute path>". Encodes path
  ...
  /**
   * An implementation which wraps a String URI. This URI can be opaque or hierarchical, but we
   * extend AbstractHierarchicalUri in case we need the hierarchical functionality.
   */
  ...
  /**
   * Creates an opaque Uri from the given components. Encodes the ssp which means this method cannot
   * be used to create hierarchical URIs.
   *
   * @param scheme of the URI
   * @param ssp scheme-specific-part, everything between the scheme separator (':') and the fragment
   ...
   *
   * <p>An absolute hierarchical URI reference follows the pattern: {@code
   * <scheme>://<authority><absolute path>?<query>#<fragment>}
   *
   * <p>Relative URI references (which are always hierarchical) follow one of two patterns: {@code
   * <relative or absolute path>?<query>#<fragment>} or {@code //<authority><absolute
   * path>?<query>#<fragment>}
   *
   * <p>An opaque URI follows this pattern: {@code <scheme>:<opaque part>#<fragment>}
   *
  ...
  public static final class Builder {
     ...
     *
     * <p>If the path is not null and doesn't start with a '/', and if you specify a scheme and/or
     * authority, the builder will prepend the given path with a '/'.
     */
   ...
   *
   * <p><strong>Warning:</strong> Prior to Jelly Bean, this decoded the '+' character as '+' rather
   * than ' '.
   *
   * @param key which will be encoded
   * @throws UnsupportedOperationException if this isn't a hierarchical URI
  ...
  /**
   * Searches the query string for the first value with the given key and interprets it as a boolean
   * value. "false" and "0" are interpreted as <code>false</code>, everything else is interpreted as
   * <code>true</code>.
   *
  ...
  /**
   * Return an equivalent URI with a lowercase scheme component. This aligns the Uri with Android
   * best practices for intent filtering.
   *
   * <p>For example, "HTTP://www.android.com" becomes "http://www.android.com"
   *
   * <p>All URIs received from outside Android (such as user input, or external sources like
   * Bluetooth, NFC, or the Internet) should be normalized before they are used to create an Intent.
   *
   * <p class="note">This method does <em>not</em> validate bad URIs, or 'fix' poorly formatted URIs
   * - so do not use it for input validation. A Uri will always be returned, even if the Uri is
   * badly formatted to begin with and a scheme component cannot be found.
   *
  ...
  /**
   * Decodes '%'-escaped octets in the given string using the UTF-8 scheme. Replaces invalid octets
  ...
  /**
   * Immutable wrapper of encoded and decoded versions of a URI part. Lazily creates the encoded or
   * decoded version from the other.
   */
  ...
  /**
   * Immutable wrapper of encoded and decoded versions of a path part. Lazily creates the encoded or
   * decoded version from the other.
   */
  static class PathPart extends AbstractPart {
    ...
    /**
     * Cached path segments. This doesn't need to be volatile--we don't care if other threads see
     * the result.
     */
    ...
    /**
     * Prepends path values with "/" if they're present, not empty, and they don't already start
     * with "/".
     */
